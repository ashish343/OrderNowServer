...
public class RestaurantEventsHandler {
	...
	private static RestaurantData generateRestaurantData(
			HttpServletRequest request) {
		RestaurantData data;

		String restuarantName = request
				.getParameter(UrlParameter.RESTUARANT_NAME.toString());
		String restuarantAddress = request
				.getParameter(UrlParameter.RESTUARANT_ADDRESS.toString());
		String restuarantConatctInfo = request
				.getParameter(UrlParameter.RESTUARANT_CONTACT_INFO.toString());
		String restuarantTableNumber = request
				.getParameter(UrlParameter.RESTUARANT_TABLES.toString());
	...
	public static void handleOrderReceived(HttpServletResponse response,
		...
		if (isDebug) {
			outputStream
					.write(("\nCalling notify Channel for Channel::" + channel)
							.getBytes());
		}
		String message = "{\"channels\":[\"" + channel
				+ "\"],\"data\": {\"action\":\"com.example.UPDATE_STATUS\","
				+ "\"message\": \""
				+ RestauntantMessage.ORDER_RECEIVED_MESSAGE.toString() + "\"}}";
	...
	public static void handleOrderCompleted(HttpServletResponse response,
		...
		for (String customer : customerList) {
			ParseNotificationHelper.registerChannel(customer, "", outputStream);
		}
	}
	...
	public static void handleBill(HttpServletResponse response,
		...
		String message = "{\"channels\":[\"" + channel
				+ "\"],\"data\": {\"action\":\"com.example.UPDATE_STATUS\","
				+ "\"message\": \""
				+ RestauntantMessage.BILL_GENERATED_MESSAGE.toString() + "\"}}";
		ParseNotificationHelper.notifyChannel(channel, message, outputStream);
	}
	...
	public static void handleModifyOrder(HttpServletResponse response,
		...
		if (dishIds != null && !dishIds.isEmpty()) {
			...
			String message = "{\"channels\":[\""
					...
					+ "\"],\"data\": {\"action\":\"com.example.UPDATE_STATUS\","
					+ "\"message\": \""
					+ RestauntantMessage.MODIFY_ORDER_MESSAGE.toString()
					+ "\",\"dishIds\": \"" + dishIds + "\"}}";
			ParseNotificationHelper.notifyChannel(channel, message,
					outputStream);
		}
	}
